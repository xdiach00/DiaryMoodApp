...
internal fun rememberChartStyle(columnChartColors: List<Color>, lineChartColors: List<Color>): ChartStyle {
    val themeModeRepository = get<ThemeModeRepository>()
    val themeMode = themeModeRepository.themeMode.value
    val isAppInDarkTheme = if (themeMode == ThemeMode.SYSTEM) isSystemInDarkTheme() else themeMode == ThemeMode.DARK

    return remember(columnChartColors, lineChartColors, isAppInDarkTheme) {
        val defaultColors = if (isAppInDarkTheme) DefaultColors.Dark else DefaultColors.Light
        ChartStyle(
            ChartStyle.Axis(
